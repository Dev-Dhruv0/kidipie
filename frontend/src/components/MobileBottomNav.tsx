...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ onOpenNewPost }) => {
  const navItems = [
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[var(--bg-app)] border-t-2 border-[var(--border-subtle)] shadow-[0px_-4px_16px_rgba(93,57,223,0.08)] flex justify-around items-center px-2 py-2 pb-safe">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center justify-center p-1.5 transition-transform active:scale-95 ${
                isActive ? 'text-[var(--primary)] scale-105 font-extrabold' : 'text-[var(--text-secondary)] font-bold'
              }`
            }
          >
            {({ isActive }) => (
              <>
                <Icon className={`w-5 h-5 ${isActive ? 'fill-[var(--primary)]' : ''}`} />
      ...
      >
        ...
        <span className="text-[10px] font-bold text-[var(--primary)] mt-0.5">Post</span>
      </button>
